...
- Style entry: import `@bagaking/jsoneditor/style.css` from your app entry when you need the component's built-in styles. `dist/style.css` only covers built-in component styles; consumer Tailwind classes passed through `decorationConfig` must be generated by your app's own Tailwind build.
...
The `italic`, `bg-blue-100/30`, `rounded`, `px-1`, `text-green-600`, and `font-medium` classes below are Tailwind classes from the consuming app. Make sure your Tailwind content scan or safelist includes them; the package `dist/style.css` does not generate styles for user-defined classes.
