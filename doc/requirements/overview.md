...
> - 术语解释请参考 [术语表](../../docs/design/glossary.md)
...
1. **高度可定制**
   ...
   - 提供丰富的交互回调机制：
     ...
     * 编辑事件：值变更、格式化等
     * 自定义事件：支持注册自定义事件类型
   - 支持特殊字段的语法校验和格式化 [➜ 详见术语表](../../docs/design/glossary.md#特殊字段-special-field)
   - 提供字段级别的编辑控制：
     * 访问控制：只读、条件编辑
     * 值校验：格式、范围、依赖关系
     * 编辑提示：自动完成、值建议、错误提示
...
### 2. 匹配系统
> 详细的匹配规则请参考 [术语表](../../docs/design/glossary.md#匹配规则-matching-rule)
...
### 3. 显示控制
> 具体使用场景请参考 [用户故事](./stories.md#核心场景)
...
### 5. 特殊字段支持
> 字段类型定义请参考 [术语表](../../docs/design/glossary.md#特殊字段-special-field)
...
### 第三阶段：高级特性（M3）
> 具体场景请参考 [用户故事](./stories.md#3-扩展编辑器功能)
...
3. **用户体验**
   > 详细的用户体验要求请参考 [用户故事](./stories.md#可用性要求)
   - 操作响应及时，无明显卡顿
   - 遵循主流编辑器的快捷键习惯
   - 提供清晰的错误提示和操作引导
